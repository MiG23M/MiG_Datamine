...
let function sampleCurveY(t, ay, by, cy) {
  return ((ay * t + by) * t + cy) * t
}

let function sampleCurveDerivativeX(t, ax, bx, cx) {
  return (3.0 * ax * t + 2.0 * bx) * t + cx
}

let function solveCurveX(x, epsilon, ax, bx, cx) {
  local t0
  local t1
  local t2 = x
  local x2
...
let function solve(t, p1x, p1y, p2x, p2y) {
